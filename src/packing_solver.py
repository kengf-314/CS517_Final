...
def solve_square_packing(container_size: int, square_sizes: list[int]) -> list[tuple[int, int]] | None:
    result = solve_packing(squares_instance(container_size, square_sizes))
    if not result.is_sat:
        return None
    return [(placement.x, placement.y) for placement in result.placements]


def validate_result(instance: PackingInstance, result: PackingResult) -> None:
...
def solve_packing(instance: PackingInstance) -> PackingResult:
    _require_z3()
    _validate_instance(instance)

    ordered_pieces = tuple(sorted(instance.pieces, key=lambda piece: (-piece.area, piece.id)))
    stats_base = _base_stats(instance, ordered_pieces)
    if instance.total_piece_area > instance.container_area:
    ...
    if instance.timeout_seconds is not None:
        solver.set(timeout=instance.timeout_seconds * 1000)
    ...
    for piece in ordered_pieces:
        can_rotate = _piece_can_rotate(instance.mode, piece)
    ...
    for i, _piece in enumerate(ordered_pieces):
        ...
        solver.add(x_vars[i] + widths[i] <= instance.container_width)
        solver.add(y_vars[i] + heights[i] <= instance.container_height)
    ...
    if instance.symmetry_breaking and ordered_pieces:
        # Reflection symmetry lets the largest piece be placed in the lower-left half
        # without changing satisfiability.
        solver.add(2 * x_vars[0] + widths[0] <= instance.container_width)
        solver.add(2 * y_vars[0] + heights[0] <= instance.container_height)
    ...
    stats = PackingStats(
        ...
        total_piece_area=instance.total_piece_area,
        container_area=instance.container_area,
    )

    if check_result == sat:
        placements = _model_to_placements(solver.model(), ordered_pieces, x_vars, y_vars, rot_vars)
        result = PackingResult(instance.name, instance.mode, "SAT", tuple(placements), elapsed, stats)
        validate_result(instance, result)
        return result
    if check_result == unsat:
        return PackingResult(instance.name, instance.mode, "UNSAT", (), elapsed, stats)
    return PackingResult(instance.name, instance.mode, "UNKNOWN", (), elapsed, stats, reason=str(check_result))


def _model_to_placements(
    model: ModelRef,
    pieces: tuple[Piece, ...],
    x_vars: list[Any],
    y_vars: list[Any],
...
def _piece_can_rotate(mode: Mode, piece: Piece) -> bool:
    return mode == "rectangles_rotation" and piece.rotatable and not piece.is_square


def _base_stats(instance: PackingInstance, ordered_pieces: tuple[Piece, ...]) -> PackingStats:
    rotation_variables = sum(_piece_can_rotate(instance.mode, piece) for piece in ordered_pieces)
    return PackingStats(
        num_pieces=len(ordered_pieces),
...
def _validate_instance(instance: PackingInstance) -> None:
    if instance.container_width <= 0 or instance.container_height <= 0:
        raise ValueError("Container dimensions must be positive")
    if instance.mode not in {"squares", "rectangles_no_rotation", "rectangles_rotation"}:
        raise ValueError(f"Unsupported mode: {instance.mode}")
    if not instance.pieces:
    ...
    for piece in instance.pieces:
        ...
        if instance.mode == "squares" and not piece.is_square:
